document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const studentResponse = await fetch(`http://54.226.152.102:3000/api/students/${studentId}`, {
            method: 'GET',
            headers: {
        ...
        if (studentResponse.ok) {
            ...
            const enrollmentsResponse = await fetch(`http://54.226.152.102:3000/api/enrollments/student/${studentId}`, {
                headers: {
                    'Authorization': `Bearer ${studentId}`
                }
            });
